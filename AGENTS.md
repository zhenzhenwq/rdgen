...
- The current deployed application release is `c304e1573be862d719f9a3454b5304b267182f35` (tree `bf3dd672afd11af6c8c300a7a15864a3a23e061c`). It adds a read-only build-history dashboard: staff see all tasks, members are database-scoped to their own tasks, and filters cover user, status, platform, time, name, UUID, and GitHub run ID.
...
- The server application is built from `c304e15`; later documentation-only commits do not change the live image. Push-triggered Docker failures remain limited to the repository's existing Docker Hub login step and do not dispatch client builds or affect the directly deployed server image.
...
- Source archive SHA-256: `b9899fd6cc2ae87ecf0ad7af080e0a879cc55a7db7b0b343b09bc81388e85a85`. Verified live image: `sha256:c22c67d9dd83c555f04688d02ff0a9afc4521c80514cf7a476fbd99e14f1af6d`.
- The service is `running`, `healthy`, restart count `0`, and has no post-deploy error fingerprints. The complete 177-test suite passed locally and again in the candidate image running Django `5.2.16`; the copied-production migration rehearsal, authenticated build-record rendering, login/registration rendering, static-asset hashes, and isolated candidate instance also passed before cutover.
...
- Keep `/opt/rdgen-backups/20260806-112556-c304e1573be8`, `/opt/rdgen-previous-20260806-112556-c304e1573be8`, and image tag `rdgen-rollback:20260806-112556-c304e1573be8` for the current rollback. The backup includes online and final SQLite snapshots, the prior environment and Nginx configuration, the exact source archive, build/test logs, and image metadata. Retain `20260806-095234-519e12ac7652` and earlier rollback sets as fallbacks.
