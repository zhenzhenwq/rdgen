...
- The current deployed application release is `19141a9b8d063b23b64fd4147500c3b9ec9c7e4e` (tree `7eb56cc2782695f17c30242a14eaee5e032bfe2c`). It adds the approved two-column login page and generated build-flow illustration on top of the membership release. All authenticated generator, user, activation-code, result, and registration templates remain byte-for-byte aligned with the preceding deployed application where applicable.
...
- The server application is built from `19141a9`; later documentation-only commits do not change the live image. Push-triggered Docker failures remain limited to the repository's existing Docker Hub login step and do not dispatch client builds or affect the directly deployed server image.
...
- Source archive SHA-256: `63393cbf94e9016d25618b3889071206e11570afeaa5928315e5d906ccd1458c`. Verified live image: `sha256:8849d63de8389d1c6f9b67ef4400da64fa1146bcd68596629699818c5504d2c6`.
...
- `.env`, `data`, `exe`, `png`, and `temp_zips` were preserved. `migrate --check` and SQLite `quick_check` pass with 3 users and all 39 historical task rows retained. Database and GitHub gates reported zero active client builds before, during, and after the switch.
- Keep `/opt/rdgen-backups/20260806-084425-19141a9b8d06`, `/opt/rdgen-previous-20260806-084425-19141a9b8d06`, and image tag `rdgen-rollback:20260806-084425-19141a9b8d06` for the current rollback. The backup includes online and final SQLite snapshots, the prior environment and Nginx configuration, the exact source archive, build/test logs, and image metadata. Retain `20260805-171543-16389d36cd35` and earlier rollback sets as fallbacks.
