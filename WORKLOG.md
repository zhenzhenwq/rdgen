...
- Administrative MSI extraction confirmed a complete payload and successfully decoded `custom_.txt`: the ID server is correct, no top-level relay key exists, and `override-settings.relay-server` is present and empty. The remaining check is to run this exact portable EXE on two Windows devices, confirm a relayed session in hbbr logs, and then perform one controlled relay failure/reconnection test.
