...
def customize(path: Path, filename: str, app_name: str) -> None:
    ...
    if preload_line in text:
        expected_exec = f"    exec: usr/share/{filename}/{filename}\n"
        expected_id = f"    id: {json.dumps(filename)}\n"
        expected_icon = f"    icon: {json.dumps(filename)}\n"
        expected_name = f"    name: {json.dumps(app_name, ensure_ascii=False)}\n"
        if any(
            expected not in text
            for expected in (expected_exec, expected_id, expected_icon, expected_name)
        ):
            raise SystemExit("AppImage shim is present but the executable path is invalid")
        print(f"Beijing compatibility shim is already enabled in {path}")
        return
    replacements = {
    ...
    print(f"Enabled Beijing compatibility shim in {path}")


def main() -> None:
    args = parse_args()
    customize(Path(args.recipe), args.filename, args.app_name)
