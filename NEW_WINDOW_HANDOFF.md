...
- Current deployed application release: `e7e4740f698643c3acfc7f4e3ee015290a45bc7e` (`Add hide tray client setting`, tree `b64fe2c202a97762496fa0f6991353047c76d534`). It includes the prior account/download/artifact, entitlement, and embedded-icon behavior plus the dedicated hide-tray option and wallpaper opt-in default.
- The live application is built from `e7e4740`. Documentation-only commits after that application commit do not change the server image. Push-triggered Docker failures remain at the known Docker Hub login step and do not dispatch RustDesk client workflows.
...
- Django `5.2.16`: 144 tests pass locally and in the candidate release path. Coverage includes the entitlement matrix plus hide-tray defaults, serialization, both manual textareas, hyphen/underscore aliases, precedence, unsupported Android/standard Linux submissions, and forged POST rejection. System checks, migration-drift checks, and `git diff --check` pass.
...
- The live generator was deployed from tree `b64fe2c202a97762496fa0f6991353047c76d534`. Windows x64 task success still requires exact persisted `.exe` and `.msi` receipts whose size and SHA-256 match the final files; uploads are staged and atomically committed, retries cannot replace immutable content, and terminal status callbacks cannot revive or bypass finalization.
...
- Application source commit: `e7e4740f698643c3acfc7f4e3ee015290a45bc7e`; application tree: `b64fe2c202a97762496fa0f6991353047c76d534`.
- Deployment ID: `20260723-162446-e7e4740f6986`.
- Source archive SHA-256: `ab2afb959829927dc1484768df9ccd258bdaf9e1739027e354e9aa47a4c269b5`.
- Live image: `sha256:0caa747a28c97703f3c6568582800d7ead2d55e352e3425584a3e74f3fe9715c`.
- Live container is `rdgen-rdgen-1`, verified `running`, `healthy`, restart count `0`, with zero traceback/critical/worker-timeout log fingerprints. Ordinary users see their actual entitlement; administrators remain unlimited. Other Settings now exposes a dedicated “隐藏系统托盘图标” toggle, while it and “传入会话期间移除壁纸” both default off. Legacy `hide-tray` and `hide_tray` manual entries are normalized without platform bypasses. Windows x64 tasks still require exact EXE/MSI finalization, and password guidance remains relaxed and Chinese.
...
- Root-only rollback material is under `/opt/rdgen-backups/20260723-162446-e7e4740f6986`, `/opt/rdgen-previous-20260723-162446-e7e4740f6986`, and `rdgen-rollback:20260723-162446-e7e4740f6986`. The preceding `20260722-204633-4da7e535f35a` and earlier rollback sets remain present.
...
- No fresh public client generation has been intentionally submitted against `e7e4740`; task 27 verifies legacy EXE/MSI rendering, while hide-tray, artifact, and entitlement enforcement are covered by backend/workflow tests. A real generation remains an explicit user-approved follow-up.
