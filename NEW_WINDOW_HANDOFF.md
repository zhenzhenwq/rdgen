...
- Total integration closeout has since advanced substantially. Server branch `rdsmart/phase2-integration-116` is at local commit `9ddf237` after `248cacb` (separate relay-supervisor UID/IPC composition), `a4e2f0c` (audited drain/force-offline/resume convergence), and `9ddf237` (strict read-only `node list`). Rocky 8 builds and focused Linux tests pass. The full 684-test Linux library run previously reported 649 passes and 35 environment/security-fixture failures; the affected integration, protocol, local-admin, CLI, supervisor, and packaging gates are green, but the full suite must not be described as green.
- Generator commits `fd13daf` and `c99801e` add the off-by-default `smartMultiRelay` form/history/migration path, exact 1.4.9 and strict-WSS validation, true-only application in Windows x64/x86, Linux, and Android workflows, locked root plus `hbb_common` production patches, fixture exclusion, reason-code compatibility fallback, and network/lifecycle invalidation. On 2026-08-20, all 200 Django tests, system checks, migration-drift checks, two workflow tests, and the complete offline apply/disable/wrong-baseline smoke contract passed. No GitHub workflow was dispatched and no artifact was built.
- The VM node is already enrolled with credential generation 1 and must not be re-enrolled. Its controller DB is authoritative. No agent or hbbr process currently runs there. A runnable `max-applied 2` slice remains blocked until an explicitly authorized deployment uses an address actually owned by the host: `1.1.1.1` is not local and produced false public reachability. The VM also has an unrotated high-volume SQL log, permissive controller private-key/DB modes, and stale socket evidence that require operational remediation before a release claim. The temporary same-host phase-one services remain separate and must not be expanded without new approval.
...
6. For phase two, start from generator `c99801e` and server integration `9ddf237`; preserve the verified S1/S2/S3 evidence but keep fixture bridges out of production claims. The next release gates are real four-platform smart artifacts and install/runtime checks plus an authorized strict-WSS Linux vertical slice on an actually local endpoint. Preserve the existing VM credential/state, do not restore the stale local SQLite snapshot, do not use `1.1.1.1`, do not edit clean upstream references, and do not push, dispatch, or deploy without explicit authorization.
